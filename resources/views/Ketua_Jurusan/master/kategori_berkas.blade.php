...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Kategori Berkas Page</h1>
    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group" id="div_nama">
                        {{Form::label('text', 'Periode Waktu :', ['class' => 'awesome'])}}
                        {{Form::select('kategori',['1' => 'Sebelum Perkuliahan','2' => 'Ujian Tengah Semester','3' => 'Ujian Akhir Semester'],null,['class' => 'form-control', 'id' => 'kategori'])}}
                    </div>
...
$(document).ready( function () {
        var table = $('#table').DataTable( {
            ...
            ajax: "{{ url('/jurusan/kategori-berkas/data') }}",
                "columns": [
                    {
                        "data": "id",
                        class: "text-center",
                    ...
                    { 
                        ...
                        render : function(data){
                            if(data == 1){
                                return '<span>Sebelum Perkuliahan</span>';
                            }else if(data == 2){
                                return '<span>Ujian Tengah Semester</span>';
                            }else{
                                return '<span>Ujian Akhir Semester</span>';
                            }
                        },
                    },
                    {
    ...
    $(document).on('click', '#add', function() {
        ...
        $('#nama_berkas').val("");
        $('#kategori').val("");
        $('#form').attr('action', '{{ url('jurusan/kategori-berkas/create') }}');     
    });
    ...
    $(document).on('click', '#edit', function() {
        ...
        $.ajax({   
            type: "get",
            url: "{{ url('/jurusan/kategori-berkas/edit') }}/"+id,
            dataType: "json",
            success: function(data) {
                console.log(data[0].id);
                event.preventDefault();
                var nama_berkas=data[0].nama_berkas
                var kategori=data[0].kategori

                $('#nama_berkas').val(nama_berkas).change();
                $('#kategori').val(kategori).change();
                $('#form').attr('action', '{{ url('jurusan/kategori-berkas/update') }}/'+id);
            }
        });        
    });
    ...
    $(document).on('click', '#delete', function() {
            ...
            if (confirm("Anda Yakin ingin menghapus data?")){
                $.ajax({
                    url : "{{ url('jurusan/kategori-berkas/delete') }}/"+id,
                    success :function () {
                        location.reload();
                    }
                })
